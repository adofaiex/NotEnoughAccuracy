...
public static class Yui
{
	...
	public const ContainerDirection H = ContainerDirection.Horizontal;

	public const ContainerDirection V = ContainerDirection.Vertical;

	public const ContainerStyle N = ContainerStyle.None;

	public const ContainerStyle P = ContainerStyle.Padding;

	public const ContainerStyle B = ContainerStyle.Background;
	...
	private static Disposable ScopedEnd { get; } = new(End);

	private static Disposable ScopedPopAlign { get; } = new(PopAlign);
	...
	{
		return [.. options.OfType<GUILayoutOption>(), GUILayout.ExpandHeight(false)];
	}

	public static void AddMargin(double size)
	{
		LastMargin += Resolution.Scaled(size);
	}

	public static void Space(double size)
	{
	...
	private static void UpdateMaxSize()
	{
	...
	public static Disposable Align(double ratio = 0, double offset = 0)
	{
		AlignmentStack.Add((ratio, offset));
		return ScopedPopAlign;
	}

	public static Disposable NoAlign()
	{
		AlignmentStack.Add(null);
		return ScopedPopAlign;
	}

	private static void PopAlign()
	{
	...
	)
	{
		...
		if (
			(newContent = GUILayout.TextField(
				content,
				maxLength ?? -1,
				AdjustMargin(Resolution.InputField),
				Options(options)
			)) != content
		) result = newContent;
		UpdateMaxSize();
	...
	private class Trigger<TK, TV>(Func<TK, TV>? initializer = null)
	{
		...
		private Func<TK, TV>? Initializer { get; } = initializer;
	...
	public class SizesGroup
	{
		...
		{
			while (group.SizesPointer >= group.SizesList.Count) group.SizesList.Add(new Sizes());
			return group.SizesList[group.SizesPointer++];
		}
	...
	private class ResolutionResources
	{
		...
		private static readonly ColorGroup TextFieldBorderColors =
			new(Rgb(0x222326), Rgb(0x222326), PrimaryColors.Normal, PrimaryColors.Normal);
		...
		public ResolutionResources(int scaleTimes1M)
		{
			...
			using var _ = Mod.Info.Begin($"Resources.Load scale={Scale}");
			...
			InputField = new GUIStyle(Base)
			{
				name = "YCH Text Field",
				alignment = TextAnchor.MiddleLeft
			};

			SetupTextSize(InputField, ScaledInt(BaseTextSize), true);
			SetupBorderedRoundedRectangleBackground(InputField, Scaled(TextFieldRadius), Scaled(TextFieldBorder),
				TextFieldColors, TextFieldBorderColors);
		...
		)
		{
			...
			style.onNormal.background =
				style.normal.background = RenderRoundedRectangleImage(size, size, radius, colors.Normal);
			style.onHover.background =
				style.hover.background = RenderRoundedRectangleImage(size, size, radius, colors.Hovered);
			style.onActive.background =
				style.active.background = RenderRoundedRectangleImage(size, size, radius, colors.Active);
			style.onFocused.background =
				style.focused.background = RenderRoundedRectangleImage(size, size, radius, colors.Focused);
		}
		...
		)
		{
			...
			style.onFocused.background =
				style.focused.background = RenderSwitchImage(on, colors.Focused, buttonColors.Focused);
		}
	...
	public class Disposable(Action dispose) : IDisposable
	{
		...
		public static Disposable operator +(Disposable a, IDisposable b)
		{
